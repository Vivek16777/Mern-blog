  ...
  return (
    <Navbar className="border-b-2" fluid rounded>
      ...
      <div className="flex justify-center items-center gap-3 md:order-2 lg:mr-30">
        <Button className="w-12 hidden sm:inline h-10 cursor-pointer" color="gray" pill onClick={()=>dispatch(toggleTheme())}>
          {theme === 'light' ? <FaSun /> : <FaMoon />}
        </Button>
        {currentUser ? (
          <Dropdown arrowIcon={false} inline label={<img alt='user' src={currentUser.profilePicture} referrerPolicy="no-referrer" className="h-10 w-10 rounded-full "/>} className="relative">
